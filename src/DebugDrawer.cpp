...
OgreDebugDrawer::OgreDebugDrawer( SceneManager *scm )
{
    ...
    if( mLines == NULL )
	{
		throw ssuge::Exception(__FILE__, __LINE__, "mLines are Null");
	}
    mTriangles = new ManualObject("physics triangles");
    if( mTriangles == NULL )
	{
		throw ssuge::Exception(__FILE__, __LINE__, "mTriangles are NULL");
	}
    ...
    if( tu == NULL )
	{		
		throw ssuge::Exception(__FILE__, __LINE__, "tu is NULL");
	}
